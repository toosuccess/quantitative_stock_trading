...
def download_history_data(stock_codes):
    """下载历史数据"""
    print("正在下载历史数据...")
    ...
    for code in stock_codes:
        if code.startswith('6'):
            full_codes.append(f'SH.{code}')
        else:
            full_codes.append(f'SZ.{code}')
    ...
    for full_code in full_codes:
        try:
            ...
            print(f"  下载: {full_code}")
        except Exception as e:
            print(f"  下载失败 {full_code}: {e}")
    
    print("历史数据下载完成")

def get_qmt_kline_v2(stock_code, count=120):
    """从QMT获取K线数据 - 使用get_market_data"""
    if not HAS_XTQUANT:
        return None
    ...
    try:
        ...
        end_time = datetime.now().strftime('%Y%m%d')
        start_time = (datetime.now() - timedelta(days=200)).strftime('%Y%m%d')
        ...
        if kline_data is not None and len(kline_data) > 0:
            if hasattr(kline_data, 'items'):
                for code, df in kline_data.items():
                    if len(df) >= 20:
                        return df
            elif isinstance(kline_data, dict) and full_code in kline_data:
                df = kline_data[full_code]
                if len(df) >= 20:
                    return df
        return None
    except Exception as e:
...
def get_qmt_kline_v3(stock_code, count=120):
    """从QMT获取K线数据 - 使用get_full_tick + 历史数据"""
    if not HAS_XTQUANT:
        return None
...
def get_qmt_kline(stock_code, count=120):
    """从QMT获取K线数据 - 多种方法尝试"""
    if not HAS_XTQUANT:
        return None
    ...
    methods = [
        ('get_market_data_ex', lambda: xtdata.get_market_data_ex(
        ...
        ('get_local_data', lambda: xtdata.get_local_data(
            stock_list=[full_code],
            period='1d',
            count=count
        )),
    ...
    for method_name, method_func in methods:
        try:
            kline_data = method_func()
            
            if kline_data is not None:
                if isinstance(kline_data, dict) and full_code in kline_data:
                    df = kline_data[full_code]
                    if hasattr(df, '__len__') and len(df) >= 20:
                        return df
                elif hasattr(kline_data, '__len__') and len(kline_data) >= 20:
                    return kline_data
        except Exception as e:
            continue
...
def calculate_indicators(kline_data):
    ...
    try:
        if hasattr(kline_data, 'close'):
            close = kline_data['close'].values if hasattr(kline_data['close'], 'values') else np.array(kline_data['close'])
            volume = kline_data['volume'].values if hasattr(kline_data['volume'], 'values') else np.array(kline_data['volume'])
        else:
            return None
...
def run_selection(limit=5):
    ...
    download_history_data([item['code'] for item in unique_pool])
    
    time.sleep(3)
    
    selected_stocks = []
    
    for stock_info in unique_pool:
        stock_code = stock_info['code']
        stock_name = stock_info['name']
        stock_industry = stock_info['industry']
        
        print(f"正在分析 {stock_name}({stock_code})...", end=" ")
        
        kline = get_qmt_kline(stock_code, 120)
        if kline is None:
            print("跳过：无法获取K线数据")
            continue
    ...
    for i, stock in enumerate(result, 1):
        print(f"{i}. {stock['stock_name']}({stock['stock_code']}) - 评分: {stock['score']}, 评级: {stock['rating']}")
